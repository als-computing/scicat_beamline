...
class Als_733_Saxs_Ingester(SciCatIngesterBase):
    """SciCat ingester for ALS 7.3.3 SAXS data."""
    ...
    def ingest(
        ...
        self._logger.info(f"Created dataset with id {scicat_dataset_id} for file {txt_file.name}")
        ...
        if edf_file:
            thumbnail_file = build_waxs_saxs_thumb_733(
                image_data, self._temp_dir, edf_file.name
            )
            encoded_thumbnail = encode_image_2_thumbnail(thumbnail_file)
            self.upload_thumbnail_attachment(
                encoded_thumbnail,
    ...
    def find_sample_keywords_oct_2022(self, datasetName):
    ...
    def get_sample_id_oct_2022(self, datasetName: str):
...
def create_derived(
    ...
    sample_keywords = self.find_sample_keywords_oct_2022(derived_name)

...
def upload_data_block(
    scicat_client: ScicatClient, fs_doc: Dict, dataset_id: str, ownable: Ownable
) -> OrigDatablock:
...
def upload_raw_dataset(
    ...
    dataset = RawDataset(
        ...
        creationLocation=fs_doc.get("/measurement/instrument/instrument_name")
        or "unkown",
        ...
        dataFormat="spot",
        principalInvestigator=fs_doc.get("/measurement/sample/experiment/pi")
        or "unkown",
    ...
    for doc in spot_docs:
        try:
            ...
            with open("ingested.csv", "a") as ingested_csv:
                ingested_csv.write(
                    f"{status.spot_id}\t {status.pid}\t {status.file}\t {status.dataset_loaded}\t {status.datablock_loaded}\t {status.error}\n"
                )
